...
export function extractGlobalOptionArray(option, tag, tagToGlobalOptions) {
    ...
    return (_a = options.optionArrays[option]) !== null && _a !== void 0 ? _a : [];
}
...
export function extractGlobalURLs(option, tag, tagToGlobalOptions) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield urlsToAbsURLs(extractGlobalStrings(option, tag, tagToGlobalOptions), location.href);
    });
}
...
export function extractContext(parts, { builtInTagToUnitCompiler, style, headSTDN, footSTDN, root } = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fullSTDN = (headSTDN !== null && headSTDN !== void 0 ? headSTDN : []).concat(stdn).concat(footSTDN !== null && footSTDN !== void 0 ? footSTDN : []);
        ...
        const css = (yield urlsToAbsURLs(cssURLs, location.href))
            .map(value => `@import ${JSON.stringify(value)};`).join('');
        if (style !== undefined) {
            style.textContent = css;
        }
        for (const url of yield urlsToAbsURLs(tagToUnitCompilerURLs, location.href)) {
            try {
                Object.assign(tagToUnitCompiler, yield (new Function(`return import(${JSON.stringify(url)})`)()));
            }
            catch (err) {
                console.log(err);
            }
